export type MonthlyCheckModule =
  | 'bank'
  | 'invoice'
  | 'expense'
  | 'payroll'
...
export interface MonthlyClosingVoucher {
  id: string;
  voucherNo: string;
  date: string;
  status: 'draft' | 'review' | 'posted' | 'reversed';
  entries: MonthlyClosingEntry[];
}

export interface MonthlyClosingBankTransaction {
  id: string;
  date: string;
  status?: string;
...
export interface MonthlyClosingInvoice {
  id: string;
  invoiceDate: string;
...
export interface MonthlyClosingCheckResult extends MonthlyCheckInstance {
  systemStatus: MonthlyCheckSystemStatus;
  systemSeverity: MonthlyCheckSeverity;
...
export const DEFAULT_MONTHLY_CLOSING_TEMPLATES: MonthlyCheckTemplate[] = [
...
function template(
  ...
  allowManualConfirmation: boolean,
  route: string,
  priority: MonthlyCheckTemplate['priority'],
...
export function createMonthlyCheckInstances(
  period: string,
  templates: MonthlyCheckTemplate[] = DEFAULT_MONTHLY_CLOSING_TEMPLATES,
  previousInstances: MonthlyCheckInstance[] = [],
): MonthlyCheckInstance[] {
...
function evaluateCheck(instance: MonthlyCheckInstance, input: MonthlyClosingInput): MonthlyClosingCheckResult {
  ...
  switch (instance.code) {
    ...
    case 'input_invoice_certification': {
      ...
      if (inputInvoices.length === 0) return result(instance, 'no_data', 'info', '本月暂未发现进项发票。', 0);
      const withoutVoucher = inputInvoices.filter((item) => !item.voucherId).length;
      if (withoutVoucher > 0) return result(instance, 'warning', 'warning', `存在 ${withoutVoucher} 张进项发票尚未生成凭证或完成认证确认。`, withoutVoucher);
    ...
    case 'output_invoice_posting': {
      ...
      if (outputInvoices.length === 0) return result(instance, 'no_data', 'info', '本月暂未发现销项发票。', 0);
    ...
    case 'voucher_posting_quality': {
      ...
      if (unposted > 0) return result(instance, 'blocked', 'blocker', `存在 ${unposted} 张凭证未过账。`, unposted);
    ...
    case 'voucher_balance_quality': {
      ...
      if (unbalanced > 0) return result(instance, 'blocked', 'blocker', `存在 ${unbalanced} 张凭证借贷不平。`, unbalanced);
...
function result(
  instance: MonthlyCheckInstance,
  systemStatus: MonthlyCheckSystemStatus,
  systemSeverity: MonthlyCheckSeverity,
  systemMessage: string,
  exceptionCount: number,
  blockClosingOverride?: boolean,
...
function isVoucherBalanced(voucher: MonthlyClosingVoucher): boolean {
